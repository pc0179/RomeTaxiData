...
4. Line-of-Sight model needs to take into account bends/turns in the road network as well as being bounded by buildings (if present either side of the road). Building shapefiles are availble and are likely to form the ground truth for whether two vehicles have genuine LoS. However, 'in polygon' caculations are computationally heavy and require multiple data sources (road map and as well as building map data) in order to compute. An early work-around could be to compare the linear 'haversine' distance to the routing distance between nearby vehciles, since routed_dist>= linear_dist, the ratio, routed:linear distance could prove to be a good meta-heuristic for LoS model, since high ratios >1.1 would imply it's infeasible, but for smaller values: 1-1.1 there could be a high chance of LoS.
