...
2. Evaluate effectiveness by investigating along the lines of the following params.:
    ...
    - Do city network topology affect capability/feasibilty of system, compare for example classic radial vs grid road network structures
...
3. In the case of trip-only datasets such as NYC, entire pseudo traces are likely to be needed. To do this properly a basic traffic/link/intersection model might be required. In this case, a simple work-around could be to query Alphabets Google Maps service for a subset of trips, save the typical times and routes suggested for different days of the week. 

4. Line-of-Sight model needs to take into account bends/turns in the road network as well as being bounded by buildings (if present either side of the road). Building shapefiles are availble from osm and are likely to form the ground truth for whether two vehicles have genuine LoS. However, 'is it in polygon' caculations are computationally heavy and require multiple data sources (road map and as well as building map data) to compute. An early work-around could be to compare the linear 'haversine' distance to the routing distance between nearby vehciles. Since routed_dist>= linear_dist, the ratio, routed:linear distance could prove to be a good meta-heuristic for LoS model, since high ratios >1.1 would imply it's infeasible, but for smaller values: 1-1.1 there could be a high chance of LoS.
